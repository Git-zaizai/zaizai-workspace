...
export const LanguagesFileNames = Object.keys(bundledLanguagesBase)
/**
...
export const snippetPlugin = (md: MarkdownIt, srcDir: string) => {
  ...
  md.renderer.rules.fence = (...args) => {
    ...
    const fileNmae = src.split('.').pop() || ''
    if (!fileNmae || LanguagesFileNames.indexOf(fileNmae) === -1) {
      token.content = fileNmae ? `Code snippet path not found: ${src}` : `Invalid code snippet option`
      token.info = ''
      return fence(...args)
    }
